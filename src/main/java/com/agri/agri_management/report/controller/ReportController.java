...
public class ReportController {
    ...
    public ResponseEntity<byte[]> csv(@RequestBody ReportRequest request) {
        byte[] data = exportService.exportToCsv(reportService.generateReport(request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION,"attachment; filename=report.csv").contentType(MediaType.parseMediaType("text/csv")).body(data);
    }
    @PostMapping("/export/pdf")
    public ResponseEntity<byte[]> pdf(@RequestBody ReportRequest request) {
        byte[] data = exportService.exportToPdf(reportService.generateReport(request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION,"attachment; filename=report.pdf").contentType(MediaType.APPLICATION_OCTET_STREAM).body(data);
    }
